...
public class RedBlackTree<T extends Comparable<T>>
    ...
    public boolean insert(T data) throws NullPointerException, IllegalArgumentException {
        ...
        if (root == null) {
            ...
            //set root node of RB tree ALWAYS to be black
            root.isBlack = true;
            return returnValue;
        }
    }
    ...
    /**
     * ensure RB-Tree follow its nature after each insertion operation
     */
    private void enforceRBTreePropertiesAfterInsert(Node<T> newNode) {
      Node<T> newNodeParentSibling;

      while (newNode.parent.isBlack == false) {
        // newNode's grandparent node == null
        if (newNode.parent.parent == null) {
          break;
        }

        // parent node is right child of grandparent's node
        if (newNode.parent == newNode.parent.parent.rightChild) {
          newNodeParentSibling = newNode.parent.parent.leftChild;

          // when sibling node is Red && Non-Null
          if (newNodeParentSibling != null && newNodeParentSibling.isBlack == false) {
            newNode.parent.isBlack = true;
            newNodeParentSibling.isBlack = true;
            newNode.parent.parent.isBlack = false;
            newNode = newNode.parent.parent;
          }

          // when sibiling node is black || null
          else {
            // when child node is left child 
        ...
        // When parent node is leftchild of granfparent
        else {
          newNodeParentSibling = newNode.parent.parent.rightChild;

          // if sibiling node is Red && Non-Null
          if (newNodeParentSibling != null && newNodeParentSibling.isBlack == false) {
            newNode.parent.isBlack = true;
            newNodeParentSibling.isBlack = true;
            newNode.parent.parent.isBlack = false;
          ...
          // if sibiling node is black || null
          else {
            // when child node is left child 
            if (newNode == newNode.parent.leftChild) {
              newNode = newNode.parent;
    ...
    private void rotate(Node<T> child, Node<T> parent)
      ...
      if (child.parent == parent && child.isLeftChild()) {
        // left rotation 
        // If root node is not parent node
        if (root != parent) {
        ...
        Node<T> oldRightChild = child.rightChild;
        parent.parent = child;
        ...
        if (oldRightChild == null) {
          ...
          parent.leftChild = oldRightChild;
        }
      } else if (child.parent == parent && !child.isLeftChild()) {
        // right rotation
        // If root node is not parent node
        if (root != parent) {
          child.parent = parent.parent;
        ...
        Node<T> oldLeftChild = child.leftChild;
        parent.parent = child;
        ...
        if (oldLeftChild == null) {
          ...
          parent.rightChild = oldLeftChild;
        }
      } else {
          throw new IllegalArgumentException("ROTATION UNSUCCESSFUL.");
      }
}
  ...
  public String toString() {
    ...
    sb.append("[ ");
    if (treeNodeIterator.hasNext()) sb.append(treeNodeIterator.next());
