...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            // make copy of path, pass into it as argument without affect
            // original path object
            this.start = copyPath.start;
            this.distance = copyPath.distance;

            List<T> newSequence = new LinkedList<>();
            for (T data : copyPath.dataSequence) {
                newSequence.add(data);
            }

            this.dataSequence = newSequence;

            //extend path by the edge object extendby
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        // throw exception when no vertex at either start or end
        if (!containsVertex(start) || !containsVertex(end)) {
            throw new NoSuchElementException("ERROR! INVALID START/END VERTEX");
        }

        // Use hashtable track visited vertices
        Hashtable<T, Vertex> passedVertex = new Hashtable<T, Vertex>();
        
        PriorityQueue<Path> passedPath = new PriorityQueue<Path>();
        PriorityQueue<Path> nextTargetPath = new PriorityQueue<Path>();

        // store 1st original path into "passedPath" priority queue
        passedPath.add(new Path(vertices.get(start)));

        // while there is no path passed already
        while (!passedPath.isEmpty()) {
            // get shortest path currently
            Path currentPath = passedPath.poll();
            Vertex currentVertex = currentPath.end;

            // traverse all edges leaving from current vertex, add to passedpath
            for(Edge e : currentVertex.edgesLeaving){
                if(!passedVertex.containsKey(e.target.data)){
                    Path nextPath = new Path(currentPath, e);
                    passedPath.add(nextPath);
                    if(nextPath.start.data.equals(start) && nextPath.end.data.equals(end)){
                        nextTargetPath.add(nextPath);
                    }
                }
            }
            passedVertex.put(currentVertex.data, currentVertex);
        }

        // search fails to find any path from the start to end vertices
        if (nextTargetPath.isEmpty()) {
            throw new NoSuchElementException("ERROR! NO PATH FOUND");
        }
        
        // return shortest path from start to end finally
        return nextTargetPath.poll();
    }
